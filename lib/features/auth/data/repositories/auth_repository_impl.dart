...
class AuthRepositoryImpl implements AuthRepository {
  ...
  final UserLocalDBDataSource dbDataSource;
  ...
  Future<Either<Failure, User>> signIn(String email) async {
    try {
      ...
      final user = await dbDataSource.getUser(email);
  ...
  Future<Either<Failure, User>> signUp(User user) async {
    try {
      ...
      final result = await dbDataSource.saveUser(UserModel(firstName: user.firstName, lastName: user.lastName, email: user.email));
      // Cache the user and sign in
